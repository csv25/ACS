...
rfe_model <- rfe(trainData_SMOTE.ROSE[, -ncol(trainData_SMOTE.ROSE)], 
                 trainData_SMOTE.ROSE$Class,
                 sizes = c(5, 10, 15, 20), 
                 rfeControl = rfe_control, 
                 metric = ifelse(is.factor(trainData_SMOTE.ROSE$Class), "Accuracy", "Kappa"))
...
rf_model <- randomForest(Class ~ ., data = trainData_SMOTE.ROSE[, c(selected_features_rfe, "Class")],
                         ntree = 1500, 
                         importance = TRUE, 
                         classwt = c(1, 15),
                         maxnodes = 250,
...
lr_model <- train(Class ~ ., data = trainData_SMOTE.ROSE[, c(selected_features_rfe, "Class")],
                  method = "glm",
                  family = "binomial",
...
svm_model <- svm(Class ~ ., data = trainData_SMOTE.ROSE[, c(selected_features_rfe, "Class")], 
                 kernel = "radial", 
                 gamma = c(0.01, 0.1, 1, 10, 20),
                 cost = c(0.1, 1, 10, 20, 30, 40,  80),
                 degree = 3,
...
nb.grid <- expand.grid(usekernel = c(TRUE, FALSE),
                       laplace=c(0,1),
                       adjust = c(0.15, 0.25, 0.50, 0.75, 1))
...
nb.model <- train(Class ~., 
                  data = trainData_SMOTE.ROSE[, c(selected_features_rfe, "Class")],
                  method = 'naive_bayes',
                  tuneGrid = nb.grid,
...
knn_predictions <- knn(train = trainData_SMOTE.ROSE[, c(selected_features_rfe, "Class")], 
                       test = testData_selected, 
                       cl = trainData_SMOTE.ROSE$Class, 
                       k = 3)
