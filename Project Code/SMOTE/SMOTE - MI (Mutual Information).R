...
# 1 . 
rf_model <- randomForest(Class ~ ., data = trainData_SMOTE[, c(selected_features_mi, "Class")], 
...
print(rf_model)
rf_train_predictions <- predict(rf_model, trainData_SMOTE[, c(selected_features_mi, "Class")])
...
print(lr_model)
lr_train_predictions <- predict(lr_model, trainData_SMOTE[, c(selected_features_mi, "Class")])
...
# 1. 
rf_balanced_model <- ranger(Class ~ ., data = trainData_SMOTE[, c(selected_features_mi, "Class")], 
...
rf_balanced_train_predictions <- predict(rf_balanced_model, trainData_SMOTE[, selected_features_mi])$predictions
rf_balanced_train_predictions <- factor(rf_balanced_train_predictions, levels = levels(trainData_SMOTE$Class))
...
# 3. 
nb_model <- naiveBayes(Class ~ ., data = trainData_SMOTE[, c(selected_features_mi, "Class")], laplace = 1)
...
cart_model <- rpart(Class ~ ., data = trainData_SMOTE[, c(selected_features_mi, "Class")], 
                    method = "class", control = rpart.control(cp = 0.01))
print(cart_model)

cart_train_predictions <- predict(cart_model, trainData_SMOTE[, c(selected_features_mi, "Class")], type = "class")
...
knn_predictions <- knn(train = trainData_SMOTE[, selected_features_mi], 
                       test = testData_selected[, selected_features_mi], 
...
train_matrix <- xgb.DMatrix(data = as.matrix(trainData_SMOTE[, selected_features_mi]), 
                            label = as.numeric(trainData_SMOTE$Class) - 1)
test_matrix <- xgb.DMatrix(data = as.matrix(testData_selected[, selected_features_mi]))
