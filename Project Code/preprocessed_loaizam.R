###################################### DATA : 
# Working directory - Setting/Getting
getwd()
# Set working directory
setwd('/Users/melanieloaiza/Desktop/BU - Data Science /Spring 2025/MET CS699 - Data Mining/project_assignment')

library(dplyr)
library(caret)
library(tidyr)

library(naniar)

library(ggplot2)
library(gridExtra)
library(ggcorrplot)

df <- read.csv('project_data.csv') 
head(df)  

dim(df)
str(df)
summary(df) 

# 1.  Reduce variables with zero variance 1 : 
df1 <- df  
zerovar <- nearZeroVar(df1, names = TRUE) 
print(zerovar) 

df1 <- df1 %>% select(-all_of(zerovar))  

dim(df1)  
str(df1)
summary(df1) 

# 2. Check for duplicates  :  
df2 <- df1 
df2_duplicates <- df2[duplicated(df2), ]
print(nrow(df2_duplicates))  

# 3. Missing Values : 
df3 <- df2 
n_miss(df3)  

miss_var_summary(df3) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

gg_miss_var(df3) +
  theme_minimal() +  
  theme(axis.text.y = element_text(size = 5)) 

# Missing values : Variables with NA doesnt have a meaning , OK to drop : 
df4 <- df3

#zerovar4 <- nearZeroVar(df4, names = TRUE) 
#print(zerovar4) 

## Varibles that are ok to delete (identifier and person number) 
df4 <- df4[, !names(df4) %in% c("SERIALNO", "SPORDER")]

dim(df4)
summary(df4)
str(df4)

zerovar4 <- nearZeroVar(df4, names = TRUE) 
print(zerovar4) 

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

## Variables with low cardinality :  
df4$GCM[is.na(df4$GCM)] <- "0"
df4$SFR[is.na(df4$SFR)] <- "0"
df4$GCR[is.na(df4$GCR)] <- "0"
df4$ESP[is.na(df4$ESP)] <- "0"
df4$NOP[is.na(df4$NOP)] <- "0"

zerovar_4 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_4) 

df4 <- df4 %>% select(-c(GCM, SFR, GCR, ESP, NOP)) 

zerovar_4 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_4) ## No variance 0 

## Reorganized variables-categories : 
df4$MLPA[is.na(df4$MLPA)] <- "0"  
df4$MLPA[df4$MLPA == "0"] <- "1"  
df4$MLPA[df4$MLPA == "1"] <- "2"

df4$MLPB[is.na(df4$MLPB)] <- "0"
df4$MLPB[df4$MLPB == "0"] <- "1"
df4$MLPB[df4$MLPB == "1"] <- "2"

df4$MLPE[is.na(df4$MLPE)] <- "0"
df4$MLPE[df4$MLPE == "0"] <- "1"
df4$MLPE[df4$MLPE == "1"] <- "2"

df4$MLPCD[is.na(df4$MLPCD)] <- "0"
df4$MLPCD[df4$MLPCD == "0"] <- "1"
df4$MLPCD[df4$MLPCD == "1"] <- "2"

df4$MLPFG[is.na(df4$MLPFG)] <- "0"
df4$MLPFG[df4$MLPFG == "0"] <- "1"
df4$MLPFG[df4$MLPFG == "1"] <- "2"

zerovar_5 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_5) ## [1] "MLPA"  "MLPB"  "MLPCD" "MLPE"  "MLPFG"

df4 <- df4 %>% select(-c(MLPA, MLPB, MLPCD, MLPE, MLPFG)) 

zerovar_6 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_6) ## No variance 0 

dim(df4)

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

df4$VPS[is.na(df4$VPS)] <- "0" 
df4$FOD2P[is.na(df4$FOD2P)] <- "0" 
df4$CITWP <- ifelse(is.na(df4$CITWP), 0,  
                    ifelse(df4$CITWP <= 1947, 1, 2))
df4$MIGPUMA <- ifelse(is.na(df4$MIGPUMA), 0, 
                      ifelse(df4$MIGPUMA == "00001", 1, 
                             ifelse(df4$MIGPUMA == "00002", 2, 3))) 
df4$SCHG[is.na(df4$SCHG)] <- "0" 

zerovar_7 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_7) ## [1] "FOD2P" "VPS" 
df4 <- df4 %>% select(-c(FOD2P, VPS)) 

zerovar_8 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_8) ## No variance 0 

dim(df4)

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

df4$DECADE[is.na(df4$DECADE)] <- "0" 
df4$ENG[is.na(df4$ENG)] <- "0" 
df4$FER[is.na(df4$FER)] <- "0" 
df4$LANP[is.na(df4$LANP)] <- 9999 

zerovar_9 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_9) ## No variance 0 

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

df4$DRIVESP[is.na(df4$DRIVESP)] <- "0" 
df4$SCIENGP[is.na(df4$SCIENGP)] <- "0" 
df4$FOD1P[is.na(df4$FOD1P)] <- "0" 
df4$SCIENGRLP[is.na(df4$SCIENGRLP)] <- "0"  

zerovar_10 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_10) ##[1] "FOD1P" 
df4 <- df4 %>% select(-c(FOD1P)) 

dim(df4)

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

df4$PAOC[is.na(df4$PAOC)] <- "0" 
df4$JWAP[is.na(df4$JWAP)] <- "0" 
df4$JWDP[is.na(df4$JWDP)] <- "0" 
df4$JWTRNS[is.na(df4$JWTRNS)] <- "0" 
df4$POWPUMA[is.na(df4$POWPUMA)] <- "0" 
df4$MARHT[is.na(df4$MARHT)] <- "0" 
df4$COW[is.na(df4$COW)] <- "0" 

df4$INDP[is.na(df4$INDP)] <- 9920 
df4$OCCP[is.na(df4$OCCP)] <- 9920 

df4$WRK[is.na(df4$WRK)] <- "0" 

zerovar_11 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_11) ##[1] "JWAP"

df4 <- df4 %>% select(-c(JWAP)) 
dim(df4)

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

df4$NWAB[is.na(df4$NWAB)] <- "0"  
df4$NWLA[is.na(df4$NWLA)] <- "0"  
df4$NWLK[is.na(df4$NWLK)] <- "0" 
df4$ESR[is.na(df4$ESR)] <- "0"  

zerovar_12 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_12) ## No character 

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

dim(df4)

##### NUMERIC 
df4$JWRIP[is.na(df4$JWRIP)] <- "0"  
df4$JWMNP[is.na(df4$JWMNP)] <- "0"   
df4$WKHP[is.na(df4$WKHP)] <- "0"   
df4$PERNP[is.na(df4$PERNP)] <- "0"   

### FROM NUMERIC LETS CONVERT THEM TO CATEGORICAL :  
df4$YOEP <- ifelse(is.na(df4$YOEP), 0, 
                   ifelse(df4$YOEP <= 1938, 2, 3)) 
df4$MARHYP <- ifelse(is.na(df4$MARHYP), 0,  
                     ifelse(df4$MARHYP <= 1944, 1, 2))  
df4$POVPIP <- ifelse(is.na(df4$POVPIP), 0, 
                     ifelse(df4$POVPIP <= 500, 1, 2)) 

zerovar_13 <- nearZeroVar(df4, names = TRUE) 
print(zerovar_13) ## No character 

miss_var_summary(df4) %>%
  arrange(desc(pct_miss)) %>%  
  slice_head(n = 50) %>%       
  print(n = 50)

dim(df4)

gg_miss_var(df4) +
  theme_minimal() +  
  theme(axis.text.y = element_text(size = 5)) 

# 4. Convert numerical and categorical variables   : 
df5 <- df4
str(df5)

# CITWP , MARHYP , POVPIP - CONVERTED TO CATEGORICAL 

var_numeric <- c("PWGTP", "WAGP", "PERNP", "PINCP", "WKHP", "JWMNP", "JWRIP") 
df5 <- df5 %>%
  mutate(across(all_of(var_numeric), as.numeric)) %>%
  mutate(across(-all_of(var_numeric), as.factor))

summary(df5)

# 5 : Remove outliers : 
df6 <- df5 
df6_var_numerical <- names(df6)[sapply(df6, is.numeric)]

df6_plot <- df6 %>% pivot_longer(cols = all_of(df6_var_numerical), names_to = "variables", values_to = "values")

ggplot(df6_plot, aes(x = variables, y = values, fill = variables)) +
  geom_boxplot(outlier.size = 0.3) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 6.5), 
        axis.title.x = element_blank(),  
        axis.title.y = element_blank()) +
  scale_fill_viridis_d(guide = "none") 

#### function 1: 
func_outlier <- function(f1) {   
  q1 <- quantile(f1, 0.25, na.rm = TRUE)   
  q3 <- quantile(f1, 0.75, na.rm = TRUE)   
  iqr <- q3 - q1   
  lower_bound <- q1 - 1.5 * iqr   
  upper_bound <- q3 + 1.5 * iqr   
  return(ifelse(is.na(f1), FALSE, f1 < lower_bound | f1 > upper_bound))  
}

df6 <- df6[rowSums(sapply(df6_var_numerical, function(col) func_outlier(df6[[col]]))) == 0, ]

dim(df6) 

# 6 : Correlation : 
df7 <- df6

df7_var_numerical <- sapply(df7, is.numeric) 

corr_matrix <- cor(df7[df7_var_numerical])
print(corr_matrix)

highCorr <- findCorrelation(corr_matrix, cutoff = 0.7, names = TRUE)
print(highCorr)  ##[1] "PERNP" "WAGP"

df7 <- df7[, !names(df7) %in% c("WAGP")]
dim(df7)##[1] 3511   65 
str(df7)



